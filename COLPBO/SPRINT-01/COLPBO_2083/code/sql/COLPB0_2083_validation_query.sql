    ...
    CASE
        WHEN EXISTS (
            ...
            FROM
                ...
                JOIN ALLCONTEXTS.kickreason kickreason ON kickreason.id = erakick.kickreasonid
                AND record._context_id_ = erakick._context_id_
                JOIN ATHENANET_RAW_COMBINED.ALLCONTEXTS.kickreason athenakickreason ON kickreason.athenakickcode = athenakickreason.kickcode
            WHERE
...
WHERE
    ...
    AND EXISTS (
        SELECT
            1
        FROM
            ALLCONTEXTS.erakick fakekick
        WHERE
            fakekick.erarecordid = record.id
