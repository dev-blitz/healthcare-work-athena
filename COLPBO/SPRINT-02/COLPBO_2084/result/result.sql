...
WHERE
    ...
    AND EXISTS (
        SELECT 1
        FROM ALLCONTEXTS.erakick medicarezerotransferclose
        WHERE medicarezerotransferclose.erarecordid = record.id
            AND medicarezerotransferclose._context_id_ = batch._context_id_
            AND medicarezerotransferclose.kickcode = 'MEDICAREZEROTRANSFERCLOSE'
    )
    AND EXISTS (
        SELECT 1
        FROM ALLCONTEXTS.erakick bdefwrappealfail
        WHERE bdefwrappealfail.erarecordid = record.id
            AND bdefwrappealfail._context_id_ = batch._context_id_
            AND bdefwrappealfail.kickcode = 'FWRAPPEALFAIL'
    )
    ...
    AND EXISTS (
        SELECT 1
        FROM claimnote appealnote
        JOIN claimnote firstbdenote ON 
            firstbdenote.claimid = appealnote.claimid
            AND firstbdenote.transfertype = appealnote.transfertype
        JOIN patientinsurance appealpatientinsurance ON 
            appealnote.patientinsuranceid = appealpatientinsurance.id
        JOIN insurancepackage appealinsurancepackage ON 
            appealpatientinsurance.insurancepackageid = appealinsurancepackage.id
        JOIN patientinsurance firstbdepatientinsurance ON 
            firstbdenote.patientinsuranceid = firstbdepatientinsurance.id
        JOIN insurancepackage firstbdeinsurancepackage ON 
            firstbdepatientinsurance.insurancepackageid = firstbdeinsurancepackage.id
        WHERE appealinsurancepackage.insurancereportingcategoryid = ?
            AND firstbdeinsurancepackage.insurancereportingcategoryid = ?
            AND appealnote.claimid = ?
            AND appealnote.transfertype = ?
            AND appealnote.action = 'APPEAL'
            AND firstbdenote.action IN ('EOB', 'ERA')
            AND firstbdenote.athenakickreasonid = 14
            AND firstbdenote.created < appealnote.created
            AND (sysdate - appealnote.created) >= 7
            AND NOT EXISTS (
                SELECT 1
                FROM tcharge
                WHERE tcharge.claimid = appealnote.claimid
            ...
            AND NOT EXISTS (
                SELECT 1
                FROM claimnote
                WHERE claimnote.claimid = appealnote.claimid
