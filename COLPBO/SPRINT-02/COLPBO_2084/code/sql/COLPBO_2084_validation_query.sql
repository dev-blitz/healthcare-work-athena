SELECT
    ...
    CASE
        WHEN EXISTS (
            ...
            WHERE
                record.id = erakick.erarecordid
                AND athenakickreason.nextclaimstatus IN ('HOLD', 'MGRHOLD', 'ATHENAHOLD', 'CBOHOLD')
                AND athenakickreason.kickreasoncategoryid = 0
        ) THEN 1
        ELSE 0
    END AS nextclaimstatus,
    -- TOTAL RECORD COUNT to sort it
    (
        SELECT
            COUNT(erarecord.id)
        FROM
            ATHENANET_RAW_COMBINED.ALLCONTEXTS.erarecord erarecord
        WHERE
            erarecord._context_id_ = batch._context_id_
    ...
    (
        SELECT
            COUNT(DISTINCT erarecord1.parenterarecordid)
        FROM
            ALLCONTEXTS.erarecord erarecord1
        WHERE
            erarecord1.claimid = record.claimid
    ...
    CASE
        WHEN EXISTS (
            ...
            FROM
                erabatchcopymap
            WHERE
                batch.id = erabatchcopymap.originalerabatchid
    ...
    (
        ...
        FROM
            erabatch erabatch
            JOIN erarecord erarecord2 ON erabatch.id = erarecord2.erabatchid
            AND erabatch._context_id_ = erarecord2._context_id_
        WHERE
            erabatch.id < batch.id
...
FROM
    ...
    JOIN ALLCONTEXTS.erarecord record ON batch.id = record.erabatchid
    AND batch._context_id_ = record._context_id_
WHERE
    batch.created > '2022-01-01' -- batches after 1st January 2022
    AND batch.status <> 'DISCARDED'  -- Avoids `<> 'DISCARDED'`
    AND batch.ansiversionid = 3 -- ANSI_VERSION_ID
    AND batch.created >= DATEADD(DAY, -7, CURRENT_DATE()) -- current BDE occurred at least seven days since the appeal,
    AND EXISTS (
        SELECT 1 
        FROM (
            SELECT t.claimid
            ...
            WHERE c.kickreasonid = 20938 -- *kick_reason_id* for "FWRAPPEALFAIL" kick_code
                AND t.type = 'CHARGE'
            GROUP BY t.claimid
            HAVING SUM(t.amount) < 5000 -- Total claim charge is less than $5,000
        )
        FilteredClaims
    );
